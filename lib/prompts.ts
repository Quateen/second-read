...
- REASONING ORDER (do this before choosing the verdict): first decide population_match, intervention_match, and outcome_match, then choose the verdict CONSISTENT with them. "unsupported" REQUIRES a real discrepancy — population_match = "mismatch" (wrong/off-topic population), or the claim asserts something the trial did not evaluate. "contradicted" REQUIRES outcome_match = "mismatch" (an opposite finding). If population_match = "match" and outcome_match is not a mismatch (same-direction finding on the studied population), the verdict is "supported" — do NOT return "unsupported" when your own population/intervention/outcome comparison shows a match. A claim correctly restricted to the trial's own studied population (e.g. "in carefully selected patients meeting the trial's criteria") is population_match = "match", NOT a mismatch.
