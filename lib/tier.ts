...
export function escalateTier(sig: {
  ...
  if (!auditIncomplete) {
    ...
    // NOTE (determinism branch, 2026-07-21): a "deterministic-clean floor" that capped
    // significant/critical -> minor whenever a citation verified and no evidence driver fired was
    // REMOVED here. It was unsafe: a verified citation is equally consistent with a faithful claim
    // (CLEAN-04) and a mischaracterization (MIS-01/02/08), so the deterministic layer is NOT
    // definitive for the clean-vs-mischaracterized distinction. The floor capped voter-caught
    // mischaracterizations (MIS-01/02/08) down to MINOR — a harmful under-flag. Do NOT reintroduce a
    // citation-verified cap. CLEAN over-flag reproducibility is addressed upstream (evidence-relevance
    // accuracy + benign-band handling), never by suppressing a real alarm here.
  }
